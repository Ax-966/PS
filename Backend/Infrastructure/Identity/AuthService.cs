...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponse> RegisterAsync(string email, string password, string name)
    {
        ...
        if (!result.Succeeded)
        {
            return new AuthResponse(false, null, user.Email, user.Name, result.Errors.Select(e => e.Description));
        }
         return new AuthResponse(true, GenerateToken(user), user.Email, user.Name, Array.Empty<string>());
    ...
    public async Task<AuthResponse> LoginAsync(string email, string password)
    {
        ...
        {
            return new AuthResponse(false, null, null, null, new[]{ "Las credenciales no son válidas." });
        }
        return new AuthResponse(true, GenerateToken(user), user.Email, user.Name, Array.Empty<string>());
